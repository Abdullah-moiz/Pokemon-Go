...
import {  get_specific_card_data, update_pokemon_card } from "../services";
...
export default function UpdateData() {
    ...
    const getData =  async () => {
        const res = await get_specific_card_data(ID);
        if(res?.success){
            setValue("name" , res?.data?.name)
            setValue("familyID" , res?.data?.familyID)
            setValue("pokeDexNumber" , res?.data?.pokeDexNumber)
            setValue("generation" , res?.data?.generation)
            setValue("eveolutionState" , res?.data?.eveolutionState)
            setValue("type1" , res?.data?.type1)
            setValue("type2" , res?.data?.type2)
            setValue("weather1" , res?.data?.weather1)
            setValue("weather2" , res?.data?.weather2)
            setValue("legandary" , res?.data?.legandary)
            setValue("spawns" , res?.data?.spawns)
            setValue("aquireable" , res?.data?.aquireable)
            setValue("regional" , res?.data?.regional)
            setValue("attack" , res?.data?.attack)
            setValue("defense" , res?.data?.defense)
            setValue("stamina" , res?.data?.stamina)
            setValue("statTotal" , res?.data?.statTotal)
    ...
    const { register, formState: { errors }, handleSubmit , setValue  } = useForm<AddPokemonDatatype>({
        criteriaMode: "all"
    });
    ...
    const onSubmit: SubmitHandler<AddPokemonDatatype> = async data => {
        ...
        if (imageFile) {
            ...
            pokemonImage = uploadImageToFirebase;
        }


        if (!pokemonImage) {
            pokemonImage = selectedImage;
        }
        ...
        const res =  await update_pokemon_card(ID , finalData);
        if(res?.success){
            toast.success('Pokemon Card Updated Successfully');
            navigate('/');
            setLoading(false)
        }else{
            toast.error(res?.error);
